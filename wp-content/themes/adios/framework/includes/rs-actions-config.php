...
if( !function_exists('adios_enqueue_scripts')) {
  ...
  function adios_enqueue_scripts() {
    ...
    wp_enqueue_script( 'adios-swipebox',         get_template_directory_uri() .'/js/jquery.swipe.box.min.js',array('jquery'), ADIOS_THEME_VERSION,true);
    ...
    // register script
    wp_register_script('adios-gmapsensor',            'http://maps.google.com/maps/api/js?key=AIzaSyDjttDw_zlwsJl27N8shtKX_I2DLydSZSo',array('jquery'), ADIOS_THEME_VERSION,true);
    ...
    wp_register_script( 'adios-ytplayer',         get_template_directory_uri() .'/js/YT.player.js',array('jquery'), ADIOS_THEME_VERSION,true);
    ...
    wp_register_style( 'adios-swipebox',   get_template_directory_uri(). '/css/swipebox.min.css',null, ADIOS_THEME_VERSION);
...
if(! function_exists('adios_include_required_plugins')) {
  ...
  function adios_include_required_plugins() {
    ...
    $plugins = array(
      ...
      array(
        'name'               => esc_html__('WPide', 'adios'), // The plugin name
      ...
      array(
        ...
        'source'             =>  'http://designlazy.com/plugins/common-plugins/js_composer.zip', // The plugin source
      ...
      array(
        ...
        'source'             => 'http://designlazy.com/plugins/adios-plugins/adios-addons.zip', // The plugin source
        'required'           => true, // If false, the plugin is only 'recommended' instead of required
        'version'            => '', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
        'force_activation'   => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
        'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins

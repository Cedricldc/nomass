...
function rs_hero_video_banner( $atts, $content = '', $id = '' ) {
  ...
  wp_enqueue_script('adios-ytplayer');
  ...
  if(!empty($data_link) && $style == 'youtube'):
    $output .=  '<div class="video-iframe">';
    $output .= '<div id="bgndVideo" class="player" data-property="{videoURL:\''.$data_link.'\',containment:\'#home\',autoPlay:true, showControls:true, showYTLogo: false, mute:'.$mute.', startAt:0, opacity:1, optimizeDisplay:true}">';
    $output .= '<div class="player-mb" data-property="{videoURL:\''.$data_link.'\' ,containment:\'self\',autoPlay:true, showControls:true, showYTLogo: false, mute:'.$mute.', startAt:0, opacity:1, optimizeDisplay:false}">';
      $output .= '</div>';
      $output .= '</div>';
  endif;
